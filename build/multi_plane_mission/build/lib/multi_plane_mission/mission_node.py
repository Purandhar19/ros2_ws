import rclpy
from rclpy.node import Node

from mavros_msgs.srv import CommandBool, SetMode, WaypointPush, WaypointSetCurrent
from mavros_msgs.msg import Waypoint, State


class PlaneMission(Node):

    def __init__(self, ns):
        super().__init__(f'plane_mission_{ns}')

        self.ns = ns
        self.state = State()

        self.create_subscription(
            State,
            f'/{ns}/state',
            self.state_cb,
            10
        )

        self.arming_client = self.create_client(
            CommandBool, f'/{ns}/cmd/arming'
        )

        self.mode_client = self.create_client(
            SetMode, f'/{ns}/set_mode'
        )

        self.wp_client = self.create_client(
            WaypointPush, f'/{ns}/mission/push'
        )

        self.set_current_client = self.create_client(
            WaypointSetCurrent, f'/{ns}/mission/set_current'
        )

        self.wait_for_services()
        self.run_mission()

    def state_cb(self, msg):
        self.state = msg

    def wait_for_services(self):
        while not self.arming_client.wait_for_service(timeout_sec=1.0):
            pass
        while not self.mode_client.wait_for_service(timeout_sec=1.0):
            pass
        while not self.wp_client.wait_for_service(timeout_sec=1.0):
            pass
        while not self.set_current_client.wait_for_service(timeout_sec=1.0):
            pass

    def create_mission(self):
        wp_list = []

        def wp(cmd, lat, lon, alt):
            w = Waypoint()
            w.frame = 3
            w.command = int(cmd)
            w.is_current = False
            w.autocontinue = True
            w.x_lat = float(lat)
            w.y_long = float(lon)
            w.z_alt = float(alt)
            return w

        # TAKEOFF (must be first)
        takeoff = wp(22, 38.3148, -76.5489, 60.0)
        takeoff.is_current = True
        wp_list.append(takeoff)

        wp_list.append(wp(16, 38.3150, -76.5495, 60.0))
        wp_list.append(wp(16, 38.3155, -76.5500, 60.0))
        wp_list.append(wp(16, 38.3160, -76.5490, 60.0))

        return wp_list

    def run_mission(self):

        # Wait for FCU
        while rclpy.ok() and not self.state.connected:
            rclpy.spin_once(self, timeout_sec=1.0)

        self.get_logger().info(f"[{self.ns}] Connected!")

        # Push mission
        req = WaypointPush.Request()
        req.waypoints = self.create_mission()
        future = self.wp_client.call_async(req)
        rclpy.spin_until_future_complete(self, future)

        self.get_logger().info(f"[{self.ns}] Mission uploaded")

        # 🔥 RESET mission to waypoint 0
        set_req = WaypointSetCurrent.Request()
        set_req.wp_seq = 0
        future = self.set_current_client.call_async(set_req)
        rclpy.spin_until_future_complete(self, future)

        self.get_logger().info(f"[{self.ns}] Mission reset to WP0")

        # Set GUIDED first (safe arming mode)
        mode_req = SetMode.Request()
        mode_req.custom_mode = "GUIDED"
        future = self.mode_client.call_async(mode_req)
        rclpy.spin_until_future_complete(self, future)

        self.get_logger().info(f"[{self.ns}] GUIDED mode")

        # Arm
        arm_req = CommandBool.Request()
        arm_req.value = True
        future = self.arming_client.call_async(arm_req)
        rclpy.spin_until_future_complete(self, future)

        self.get_logger().info(f"[{self.ns}] Armed!")

        # Switch to AUTO
        mode_req = SetMode.Request()
        mode_req.custom_mode = "AUTO"
        future = self.mode_client.call_async(mode_req)
        rclpy.spin_until_future_complete(self, future)

        self.get_logger().info(f"[{self.ns}] AUTO mode")
        self.get_logger().info(f"[{self.ns}] Mission running 🚀")


def main():
    rclpy.init()

    plane1 = PlaneMission("plane1")
    plane2 = PlaneMission("plane2")

    rclpy.spin(plane1)
    rclpy.spin(plane2)

    rclpy.shutdown()


if __name__ == '__main__':
    main()